...
class Cart2PaymentController extends GetxController {
  ...
  Rx<Cart2CheckoutModel> cart2checkoutModel = Cart2CheckoutModel(
          discountPrice: 0.obs,
          onlyProductPrice: 0.obs,
          deliveryCost: 0.obs,
          userInfo: UserInfo(point: 0.obs),
          totalProductAmount: 0.obs)
      .obs;
  ...
  void init(Cart2CheckoutModel cart2checkoutModel) {
    ...
    for (CheckoutList checkout in cart2checkoutModel.checkoutList!) {
      Store tempStore = Store(
          ...
          imgUrl: checkout.storeThumbnailImagePath != null
              ? checkout.storeThumbnailImagePath!.obs
              : null);
  ...
  usePointBtnPressed() {
    ...
    if (pointController.text.isNotEmpty) {
      try {
        ...
        // check if used point is more that product price
        else if (tempUsedPoints >
            (cart2checkoutModel.value.totalProductAmount.value +
                cart2checkoutModel.value.discountPrice.value)) {
          ...
          int tempTotalProductAmount =
              cart2checkoutModel.value.onlyProductPrice.value -
                  tempUsedPoints +
                  cart2checkoutModel.value.deliveryCost.value;
  ...
  void updateTextEditControllers(cart2checkoutModel) {
    nameController.text =
        isUseNewAddress.value ? '' : cart2checkoutModel.userInfo!.name!;
    // address
    address1ZipCodeController.text = isUseNewAddress.value
        ? ''
        : cart2checkoutModel.userInfo!.addressInfo!.zipCode!;
    address2Controller.text = isUseNewAddress.value
        ? ''
        : cart2checkoutModel.userInfo!.addressInfo!.address!;
    address3Controller.text = isUseNewAddress.value
        ? ''
        : cart2checkoutModel.userInfo!.addressInfo!.addressDetail!;
    // phone number
    String phoneNumber = cart2checkoutModel.userInfo!.phone!;
    phoneFirstPartController.text =
        isUseNewAddress.value ? '' : phoneNumber.substring(0, 3);
    phoneSecondPartController.text =
        isUseNewAddress.value ? '' : phoneNumber.substring(3, 7);
    phoneThirdPartController.text = isUseNewAddress.value
        ? ''
  ...
  Future<void> searchAddressBtnPressed() async {
    ...
    cart2checkoutModel.value.deliveryCost.value =
        await _apiProvider.getDeliveryFee(
    ...
    if (cart2checkoutModel.value.totalProductAmount.value <
        minAllawablePaymentPrice) {
  ...
  paymentBtnPressed() {
    ...
    if (phoneFirstPartController.text.isEmpty ||
        phoneSecondPartController.text.isEmpty ||
        phoneThirdPartController.text.isEmpty) {
  ...
  Future<void> paymentSuccessful(Map<String, dynamic> result) async {
    ...
    bool isSuccess = await _apiProvider.postPaymentSucessfullyFinished(
      ...
      phone:
          '${phoneFirstPartController.text}${phoneSecondPartController.text}${phoneThirdPartController.text}',
    ...
    if (isSuccess) {
      ...
      Get.offAll(
          () => OrderInquiryAndReviewView(
              isBackEnable: false, hasHomeButton: true),
          arguments: false);
