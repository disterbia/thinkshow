...
class uApiProvider extends GetConnect {
  ...
  Future<List<Product>> getSimilarCat(
    ...
    String url = mConst.API_BASE_URL +
        mConst.API_USER_PATH +
        '/products/$productId/similar?offset=$offset&limit=$limit' +
        storeIdStr +
        sortStr;
    print('getProductsWithCat> url: $url');
      ...
      for (var json in jsonList) {
        ...
        for (var orderDetail in json['order_details']) {
          ...
          Product tempProduct = Product(
            ...
            normalPrice: orderDetail['normal_price'],
            priceDiscountPercent: orderDetail['price_discount_percent'],
    ...
    if (response.statusCode == 200) {
      ...
      for (var orderDetailsJSON in json['order_details']) {
        ...
        Product tempProduct = Product(
          ...
          normalPrice: orderDetailsJSON['normal_price'],
          priceDiscountPercent: orderDetailsJSON['price_discount_percent'],
  ...
  Future<bool> changePassword(
    ...
    if (response.statusCode == 200) {
      ...
      log('error:' + response.bodyString!);
      mSnackbar(message: '기존 비밀번호를 확인 해 주세요.');
      return false;
      // return Future.error(response.statusText!);
    }
  }
